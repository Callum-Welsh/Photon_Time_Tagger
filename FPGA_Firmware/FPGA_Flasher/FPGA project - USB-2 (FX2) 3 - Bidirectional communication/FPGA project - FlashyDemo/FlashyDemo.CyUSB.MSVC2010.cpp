...
static GUID GUID_CYUSB_KNJN_FX2 = {0x0EFA2C93, 0x0C7B, 0x454F, 0x94, 0x03, 0xD6, 0x38, 0xF6, 0xC3, 0x7E, 0x65};
...
///////////////////////////////////////////////////
#define nFlashyChannels 2	// 4 for Saxo-Q, 2 for the other FX2 boards
	...
	if(DataPacket.hdr1.magic_number==0x55AA && DataPacket.hdr1.version==DataPacketVersion)
	{
		if((DataPacket.hdr1.nb_channels_and_ADCfreq & 0x0F)==nFlashyChannels)
		{
			if(DataPacket.hdr2.magic_number==0x55AA && DataPacket.hdr2.version==DataPacketVersion)
			{
				...
				for(int ch=0; ch<nFlashyChannels; ch++)
				{
					printf("Channel %d: ", ch+1);
					for(int i=0; i<AcqDataLen; i++) printf("%d ", DataPacket.samples[i+AcqDataLen*ch]);
					printf("\n");
				}
			}
		}
...
void main()
{
	...
	USBDevice = new CCyUSBDevice(NULL, GUID_CYUSB_KNJN_FX2);  // make sure you choose the right GUID
	if(USBDevice->IsOpen())
	{
		GetData();

		// now we could re-trigger (call again GetData) to get more if we wanted
		// but let's just exit
	}
	else
		printf("No CyUSB device found.\nMake sure that the board is connected and the GUID is correct.\n");
